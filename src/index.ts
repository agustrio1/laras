...
function App() {
  const [count, setCount, subscribeCount] = larasState(0);
  const [theme, setTheme, subscribeTheme] = larasState('light');
  
  const toggleTheme = () => {
    setTheme(theme() === 'light' ? 'dark' : 'light');
  };

  // Create root element function
  const createApp = () => createElement(
    ...
    createElement(
      ...
      createElement(
        'p',
        { class: 'about-text' },
        "I'm a passionate frontend developer with 5 years of experience in creating responsive and user-friendly web applications."
      )
    ),
  ...
  const rerender = () => {
    ...
    if (rootEl) {
      rootEl.innerHTML = '';
      rootEl.appendChild(createApp());
      console.log(`Count Updated: ${count()}`);
    }
  };

  // Subscribe to theme changes
  subscribeTheme(rerender);
  subscribeCount(rerender);
